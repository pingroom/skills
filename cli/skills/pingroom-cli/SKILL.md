...
terminal to pair, or set `PINGROOM_TOKEN` in CI.
...
- Not connected? Bare `pingroom` in an interactive terminal starts QR pairing.
  There is deliberately no `login` subcommand. In CI, set `PINGROOM_TOKEN`;
  the CLI never prompts or draws a QR without a TTY.
